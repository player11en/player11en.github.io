    ...
    var ChromaKeyMaterial, BasicMaterial;
    var pivot;
    var markerBase;
    ...
    if(isiOSDevice() || isSafari){
        ...
        startButton.addEventListener( 'click', function () {
            loadVideo();
            controls = new DeviceOrientationControls( cubeRot );
            noSleep.enable();
        }, false );
    }
    else{
    ...
    function intitMarker(){
        ...
        cubeRot = new THREE.Mesh( geometry, material );

        cubeDebug = new THREE.Mesh( geometry, material );
        scene.add(cubeDebug);

        cubeDebug.position = new THREE.Vector3(0, 0, 0);
        controls = new DeviceOrientationControls( cubeRot );
        ...
        markerControls1 = new THREEx.ArMarkerControls(arToolkitContext, markerRoot1, {
            type: 'pattern',
            patternUrl: "patt/pattern-KP_inner.patt",
            smooth: false,
    ...
    function loadVideo(){
        ...
        if(isiOSDevice == true || isSafari == true){
            ...
            plane = new THREE.Mesh(new THREE.PlaneGeometry(1.7, 3.4), ChromaKeyMaterial);     
            pivot.add( plane );
        }
        else{
            ...
            plane = new THREE.Mesh(new THREE.PlaneGeometry(1.7, 3.4), BasicMaterial);
            ...
            video.onloadeddata = function(){
                ...
                BasicMaterial = new THREE.MeshBasicMaterial({ map : videoTexture, transparent : true, side: THREE.DoubleSide });
                plane = new THREE.Mesh(new THREE.PlaneGeometry(1.7, 3.4), BasicMaterial);
        ...
        markerBase.position.set( 0, 0, 0 );          
        markerBase.add(pivot);
    ...
    function checkMarker(){
        ...
        if (videoIsPlaying && (!markerRoot1.visible || viewWithoutMarker )) {
            ...
            if (timer >= 100 || viewWithoutMarker){
                plane.rotation.z = -cubeRot.rotation.z;
            }
    ...
    onRenderFcts.push(function(){
        if(ARInitRunning){
            ...
            {
                controls.update();
                var trackedRotation = new THREE.Vector3(cubeRot.rotation.x, cubeRot.rotation.y, cubeRot.rotation.z);
                markerBase.rotation.x = 0;
                markerBase.rotation.y = 0;
                markerBase.rotation.z = -trackedRotation.z;
                pivot.rotation.x = -cubeRot.rotation.x - 0.2;
            }
            else{
                markerBase.rotation.x = smoothedRoot.rotation.x;
                markerBase.rotation.y = smoothedRoot.rotation.y;
                markerBase.rotation.z = smoothedRoot.rotation.z;
                pivot.rotation.z = 0;
                pivot.rotation.x = -0.2;
            }
            markerBase.position.x = smoothedRoot.position.x;
            markerBase.position.y = smoothedRoot.position.y;
            markerBase.position.z = smoothedRoot.position.z;

            // document.getElementById('debug').innerHTML = deviceOrientationEnabled + "<br>"
            //                                            + cubeRot.rotation.x.toFixed(2);
