...
self.addEventListener('fetch', function (e) {
    ...
    e.respondWith(
      caches.match(e.request).then(function (respond) {
        if (respond) { // if cache is available, respond with cache
          console.log('responding with cache : ' + e.respond.url)
          return respond
        } else {       // if there are no cache, try fetching request
          console.log('file is not cached, fetching : ' + e.respond.url)
